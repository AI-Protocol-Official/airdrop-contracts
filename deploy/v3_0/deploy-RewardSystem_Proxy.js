...
module.exports = async function({deployments, getChainId, getNamedAccounts}) {
	...
	{
		// get dptToke address:
		const {rewardSystemImpl:reward_system_address,dptToken: dpt_token_address} = await getNamedAccounts();

		// get the rewardSystem Implementation
		const RewardSystem = artifacts.require("./RewardSystem");
		const impl_contract = new web3.eth.Contract(RewardSystem.abi, reward_system_address);

		// prepare proxy initialization call bytes
		const proxy_init_data = impl_contract.methods.postConstruct(dpt_token_address).encodeABI();
		...
		await deployments.deploy("RewardSystem_Proxy", {
			...
			args: [reward_system_address, proxy_init_data],
