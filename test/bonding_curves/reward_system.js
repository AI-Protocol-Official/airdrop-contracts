...
contract("Leaderboard Reward System", function(accounts) {
	...
	let rewardSystem, token;
	describe("ERC20/TOKEN reward system", function() {
		describe("deployment and inititokenzation", function() {
			beforeEach(async function() {
				token = await deploy_token_mock(a0);
			});

			describe("success, otherwise", function() {
				beforeEach(async function() {
					rewardSystem = await deploy_erc20_reward_system(a0, token.address);
				});
				...
				it("token contract address is set correctly", async function() {
					expect(await rewardSystem.erc20RewardToken()).to.equal(token.address);
				});
			});
		});
		...
		describe("user try to claim reward", function() {
			...
			it("fails, if user try to claimed more reward then allocated", async function() {
				await token.transfer(rewardSystem.address, ETH, {from: a0});
				const proof = tree.getHexProof(leaves[0]);
			...
			describe("success otherwise", function() {
				...
				it("user claim reward as expected", async function() {
					...
					expect(await token.balanceOf(rewardList[0].to)).to.be.bignumber.that.equals(new BN(rewardList[0].totalReward));
				});
	...
	describe("eth reward system", function() {
		...
		describe("reward system ACL", function() {
			beforeEach(async function() {
				token = await deploy_token_mock(a0);
				rewardSystem = await deploy_eth_reward_system(a0, token.address);
			});
			describe("when admin doesn't have ROLE_DATA_ROOT_MANAGER permission", function() {
				it("fails to update input data root", async function() {
					({root: dataRoot} = generate_merkleRoot(10));
